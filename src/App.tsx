  ...
  return (
    <div className={`h-[100dvh] w-full overflow-hidden bg-[#080910] text-white font-ios selection:bg-white/20 flex flex-col items-center transition-all duration-700 ease-ios ${isTransitioning ? "opacity-0 scale-[0.98] blur-xl" : "opacity-100 scale-100 blur-0"}`}>
      ...
      <div className="relative z-10 flex h-full w-full max-w-5xl flex-col">
        ...
        <nav className="ios-nav-float fixed bottom-[5rem] inset-x-0 z-50 mx-auto flex h-[4.35rem] w-[88%] max-w-[24rem] items-center justify-evenly gap-1 rounded-[2.15rem] border border-white/[0.18] bg-[#09090e]/55 p-1.5 shadow-[0_18px_55px_rgba(0,0,0,.45),inset_0_1px_0_rgba(255,255,255,.10)] backdrop-blur-[28px] backdrop-saturate-150 transition-all duration-500 ease-out sm:h-[4.55rem] sm:max-w-md lg:max-w-2xl lg:px-4 xl:max-w-3xl">
         {[{ id: "home", label: "Home", icon: LayoutGrid }, { id: "services", label: "Plan", icon: CreditCard }, { id: "assist", label: "Assist", icon: MessageSquare }, { id: "nearby", label: "Nearby", icon: MapPin }, { id: "activity", label: "Log", icon: ActivityIcon }].map((item) => { 
            ...
            return (
              <button key={item.id} onClick={() => setActiveTab(item.id)} className={`relative flex min-w-[3.45rem] flex-col items-center justify-center gap-1 rounded-[1.25rem] px-1 py-2 overflow-hidden transition-all duration-300 ${active ? "bg-white/[0.12] text-white scale-[1.02]" : "text-white/34 hover:text-white/72"}`}>
